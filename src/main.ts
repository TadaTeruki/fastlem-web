...
class EditorCanvas {
    ...
    start() {
        const resetButton = document.getElementById("button-removeall") as HTMLButtonElement;
        ...
        const newButton = document.getElementById("button-new") as HTMLCanvasElement;
        newButton.addEventListener("click", () => {
            this.nodeCreation = true;
        });

        const erodibilitySlider = document.getElementById("erodibility-slider") as HTMLInputElement;
        erodibilitySlider.addEventListener("input", () => {
            if (this.nodeChoosen != null) {
                ...
                this.nodes.getNode(this.nodeChoosen).erodibility = MIN_ERODIBILITY + erodibilityRatio*(MAX_ERODIBILITY-MIN_ERODIBILITY);
                this.updateContext();
            }
        });
        ...
        const oceanCheckbox = document.getElementById("ocean-checkbox") as HTMLInputElement;
        oceanCheckbox.addEventListener("change", () => {
            if (this.nodeChoosen != null) {
                this.nodes.getNode(this.nodeChoosen).isOcean = oceanCheckbox.checked;
                this.updateContext();
            }
        });
        ...
        this.canvas.addEventListener("mousedown", (e) => {
            if (this.nodeCreation) {
               ...
               this.updateContext();
                ...
                const inearest = this.nearestNodeIndex(e.clientX-this.canvas.offsetLeft, e.clientY-this.canvas.offsetTop);
                if (inearest != null) {
            ...
            if (this.nodeChoosen != null) {
                const erodibilityRatio = (this.nodes.getNode(this.nodeChoosen).erodibility-MIN_ERODIBILITY)/(MAX_ERODIBILITY-MIN_ERODIBILITY);
                erodibilitySlider.value = (erodibilityRatio*100).toString();
                oceanCheckbox.checked = this.nodes.getNode(this.nodeChoosen).isOcean;
            }
        ...
        this.canvas.addEventListener("mousemove", (e) => {
            this.updateContext(() => {
                if (this.nodeCreation) {
                    this.drawSmallRect(e.clientX-this.canvas.offsetLeft, e.clientY-this.canvas.offsetTop, "#000", null, true);
                } else {
                    const inearest = this.nearestNodeIndex(e.clientX-this.canvas.offsetLeft, e.clientY-this.canvas.offsetTop);
    ...
    drawImage() {
        ...
        let resizedArrayBuffer = new ArrayBuffer(this.canvas.width*this.canvas.height*4);
        ...
        for (let y = 0; y < bufferHeight; y++) {
            for (let x = 0; x < bufferWidth; x++) {
                let color = [0, 0, 0];
                if (this.nodes.getNode(this.nearestBuffer[y][x]).isOcean) {
                    color = [30, 150, 255];
                    if (x < oceanShadow || y < oceanShadow || (!this.nodes.getNode(this.nearestBuffer[y-oceanShadow][x-oceanShadow]).isOcean)) {
                        color = [15, 120, 240];
                    }
                } else {
                    const erodibility = this.nodes.getNode(this.nearestBuffer[y][x]).erodibility;
                    let pixel = 255 - Math.floor(255*erodibility*0.5);
                    color = [pixel, pixel, pixel];
                }

                for (let k = 0; k < this.pixelScale; k++) {
                    for (let l = 0; l < this.pixelScale; l++) {
                        resizedPixels[(x*this.pixelScale+k)*4 + (y*this.pixelScale+l)*this.canvas.width*4] = color[0];
                        resizedPixels[(x*this.pixelScale+k)*4 + (y*this.pixelScale+l)*this.canvas.width*4 + 1] = color[1];
                        resizedPixels[(x*this.pixelScale+k)*4 + (y*this.pixelScale+l)*this.canvas.width*4 + 2] = color[2];
                        resizedPixels[(x*this.pixelScale+k)*4 + (y*this.pixelScale+l)*this.canvas.width*4 + 3] = 255;
                    }
                }
            }
        }

        const imageData = new ImageData(resizedPixels, this.canvas.width, this.canvas.height);
    ...
    setImage(){
        ...
        this.nodes.getNodes().forEach((node, index) => {
            queue.queue({px: Math.ceil(node.x/this.pixelScale), py: Math.ceil(node.y/this.pixelScale), node_index: index, sqdistance: 0});
        });
        ...
        let nearestBuffer: Array<Array<number>> = new Array<Array<number>>(bufferHeight);
        ...
        while(queue.length > 0) {
            ...
            neighbours.forEach((neighbour) => {
                if (neighbour.x < 0 || neighbour.x >= bufferWidth || neighbour.y < 0 || neighbour.y >= bufferHeight) return;
                let nx = neighbour.x*this.pixelScale;
                let ny = neighbour.y*this.pixelScale;
                let sqdist =  Math.pow(nx-node.x, 2) + Math.pow(ny-node.y, 2);
                queue.queue({px: neighbour.x, py: neighbour.y, node_index: pixel.node_index, sqdistance: sqdist});
            });
        }
        this.nearestBuffer = nearestBuffer;
    }

    drawSmallRect(x: number, y: number, colorStroke: string | null, colorFill: string | null, line: boolean, shadow: boolean = true) {
        ...
        if (colorFill != null) {
            if(shadow) {
                ...
                this.ctx.fillRect(x-rectSize/2+shadowSize, y-rectSize/2+shadowSize, rectSize, rectSize);
            }
        ...
        if (colorStroke != null) {
            ...
            this.ctx.strokeRect(x-rectSize/2+innerWidth/2, y-rectSize/2+innerWidth/2, rectSize-innerWidth, rectSize-innerWidth);
        }
    }
    ...
    addNode(e: MouseEvent) {
        ...
        this.nodes.addNode({x:
            e.clientX-this.canvas.offsetLeft,
            y: e.clientY-this.canvas.offsetTop,
            erodibility: nearest ? nearest.erodibility : (MAX_ERODIBILITY+MIN_ERODIBILITY)/2,
            isOcean: nearest ? nearest.isOcean : false,
        });
